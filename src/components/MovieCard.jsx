...
export function MovieCard ({children, description, rating, picture, date}){
    ...
    return (
        <article className="relative h-[50vh] w-auto group cursor-pointer overflow-hidden lg:h-screen" onClick={openModal}>
            <img loading="lazy" src={`https://image.tmdb.org/t/p/w500${picture}`} alt={children} className="absolute top-0 left-0 h-full w-full -z-10 object-cover lg:group-hover:scale-110 transition"/>
            ...
            <div className="hidden lg:group-hover:flex bg-black/80 backdrop-blur-md flex-col items-center justify-center gap-4 absolute top-0 left-0 w-full h-full animate-fade-in animate-duration-200 px-4">
